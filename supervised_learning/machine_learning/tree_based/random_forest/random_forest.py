...
class RandomForest:
    """
    Random Forest model implemented using multiple DecisionTree models.

    Attributes:
    - n_trees (int): Number of decision trees in the forest.
    - depth (int): Maximum depth of each decision tree.
    - min_samples_split (int): Minimum number of samples required to split a node.
    - trees (List[DecisionTree]): List of DecisionTree models in the forest.

    Methods:
    - fit(X, y): Build the random forest by fitting `n_trees` DecisionTree models on bootstrap samples of the data.
    - predict(X): Make predictions using the ensemble of trained decision trees on input features X.
    """
    ...
    def __init__(self, n_trees=10, depth=10, min_samples_split=2):
        ...
        self.depth = depth
        self.min_samples_split = min_samples_split
        self.trees: List[DecisionTree] = []  # Initialize an empty list to store DecisionTree models
    
    def fit(self, X, y):
        """
        Build the random forest by fitting `n_trees` DecisionTree models on bootstrap samples of the data.
    ...
    def _bootstrap_sample(self, X, y):
        ...
        Returns:
        - tuple: Bootstrap sample (X_bootstrap, y_bootstrap).
        """
        n_samples = X.shape[0]
    ...
    def predict(self, X):
        """
        Make predictions using the ensemble of trained decision trees.
